/**
 * rudiment - CRUD resource manager
 * https://github.com/gavinhungry/rudiment
 */

(function() {
  'use strict';

  module.exports = {
    default: true,
    id: 'id',

    api: function() {
      return {
        init: function() {
          var that = this;

          // FIXME: check/generate all required indexes, not just _auto
          if (!this._auto) {
            return Promise.resolve();
          }

          return this._db.indexList().then(function(indexes) {
            if (indexes.indexOf(that._key) >= 0) {
              return;
            }

            return that._db.indexCreate(that._key);
          });
        },

        findMaxIndex: function() {
          if (!this._auto) {
            return Promise.resolve(null);
          }

          return this._db.orderBy({ index: this._key }).nth(-1).default(0);
        },

        create: function(doc) {
          var that = this;

          return this._db.insert(doc).then(function(res) {
            return res.generated_keys.length ?
              that.read(res.generated_keys[0]) :
              // FIXME: move this Error creation to caller
              Promise.reject(new Error('Document not created'));
          });
        },

        read: function(id) {
          return this._db.get(id);
        },

        find: function(props) {
          return this._db.filter(props);
        },

        update: function() {},
        delete: function(id) {
          return this._db.get(id).delete().then(function(res) {
            return res.deleted > 0;
          });
        }
      };
    }
  };

})();
